...
**Domain:** 开发者/办公工具箱网站（纯前端静态站，客户端本地计算，赛博朋克风格，需国内可访问 + 每工具独立 SEO 路由）
**Researched:** 2026-09-04
**Confidence:** MEDIUM（版本号来自 npm registry 第一方数据；部署平台行为经官方文档 + 多来源交叉验证；编辑器/搜索库选型含同行实测数据）

> **一句话结论：Vue 3.5 + Vite 8 + vite-ssg（每工具静态预渲染）+ Tailwind CSS 4 + CodeMirror 6，部署到 EdgeOne Pages（绑自定义域名，做了 ICP 备案走中国大陆节点，未备案走"全球可用区（不含中国大陆）"）。**
...
1. **愿意做 ICP 备案（推荐）**：域名在腾讯云备案（个人非经营性"工具/博客"类，约 1–3 周，需一台腾讯云轻量服务器获取备案服务码）→ EdgeOne Pages 加速区域选"中国大陆可用区" → 大陆用户命中境内 CDN 节点，这是四个候选里国内速度的**天花板**。
2. **暂不备案也能上线**：EdgeOne Pages 加速区域选"全球可用区（不含中国大陆）" + 自定义域名（无需备案，流量走香港/海外节点）——比 Vercel/GH Pages 对大陆友好得多；备案下来后切"中国大陆可用区"即可，代码零改动。
3. 兜底备选：Cloudflare Pages（免费无限带宽、免备案），国内速度平庸但稳定可访问。
4. Vercel 与 GitHub Pages **不作为主部署**：前者默认域名被墙、自定义域名国内无节点；后者 DNS 污染反复。可把 Vercel 留作纯海外镜像（可选）。
...
# 核心
pnpm add vue-router@5 @unhead/vue @vueuse/core fuse.js tailwindcss @tailwindcss/vite

# 工具库（全部按工具页面懒加载，勿进首屏）
pnpm add codemirror @codemirror/lang-json dayjs qrcode js-base64 \
  marked highlight.js browser-image-compression papaparse jszip

# 图片格式转换（WASM 懒加载）
pnpm add @jsquash/webp @jsquash/jpeg
...
pnpm add -D vite-ssg unplugin-vue-router vue-tsc vitest @vue/test-utils happy-dom
```
...
---
*Stack research for: 开发者/办公工具箱网站（纯前端静态站）*
*Researched: 2026-09-04*
